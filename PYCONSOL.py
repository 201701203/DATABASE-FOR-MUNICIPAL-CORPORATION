...
def q1():
    cursor.execute("SET SEARCH_PATH TO Municipal_Corporation; select citizen.f_name, citizen.house_no, citizen.society from citizen inner join (select peoplehavecomplains.citizenid from peoplehavecomplains inner join(select complaintid from complains where serviceid='1' and cstatus='unsolved') as r1 on r1.complaintid=peoplehavecomplains.complaintid) as r2 on r2.citizenid=citizen.citizenid where citizen.pincode='398965';")    
